...
import pushpo.emapp.fragment.PlaceAboutDetail;


public class AboutMapUtils implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener {

		private GoogleMap googleMap;
    	private PlaceAboutDetail mContext;
		...
		public AboutMapUtils(PlaceAboutDetail mContext, GoogleMap googleMap) {
...
//			}
			...
			if (markerPoints.size() >= 1) {
				...
				AboutMapUtils.DownloadTask downloadTask = new AboutMapUtils.DownloadTask();
		...
		private class DownloadTask extends AsyncTask<String, Void, String> {
			...
			protected void onPostExecute(String result) {
				...
				pushpo.emapp.maputils.AboutMapUtils.ParserTask parserTask = new pushpo.emapp.maputils.AboutMapUtils.ParserTask();
		...
		private class ParserTask extends AsyncTask<String, Integer, List<List<HashMap>>> {
			...
			protected void onPostExecute(List<List<HashMap>> result) {
				...
				for (int i = 0; i < result.size(); i++) {
					...
					lineOptions.width(8);
					lineOptions.color(ContextCompat.getColor(mContext1, R.color.color_primary));
					lineOptions.geodesic(true);

				}
				if (lineOptions == null) {
					Toast.makeText(mContext1, "No path found", Toast.LENGTH_LONG).show();
					return;
				}
		...
		protected synchronized void buildGoogleApiClient() {
			mGoogleApiClient = new GoogleApiClient.Builder(mContext1)
		...
		public void onConnected(@Nullable Bundle bundle) {
			if (ActivityCompat.checkSelfPermission(mContext1, Manifest.permission.ACCESS_FINE_LOCATION) == PackageManager.PERMISSION_GRANTED) {
		...
		public void onConnectionFailed(@NonNull ConnectionResult connectionResult) {
			Toast.makeText(mContext1, "Please check internet", Toast.LENGTH_SHORT).show();
